...
let running=false;const timer=setInterval(async()=>{if(running)return;running=true;try{await lab.tick();}catch{}finally{running=false;}},650);
...
const stop=setInterval(()=>{if(fs.existsSync(path.join(root,'STOP'))){clearInterval(timer);clearInterval(stop);server.closeAllConnections();server.close();}},500);
